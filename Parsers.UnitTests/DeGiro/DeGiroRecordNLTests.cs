...
namespace GhostfolioSidekick.Parsers.UnitTests.DeGiro
{
	public class DeGiroRecordNLTests
	{
		...
		{
			// Arrange
			var deGiroRecordNL = DefaultFixture.Create().Build<DeGiroDutchStrategy>().With(x => x.Description, description).Create();
		...
		{
			// Arrange
			var deGiroRecordNL = DefaultFixture.Create().Build<DeGiroDutchStrategy>().With(x => x.Description, string.Empty).Create();
		...
		{
			// Arrange
			var deGiroRecordNL = DefaultFixture.Create().Build<DeGiroDutchStrategy>().With(x => x.Description, "Some random description").Create();
