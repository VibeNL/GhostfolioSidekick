...
/// <summary>
/// Generic DB-backed cache service using ExternalDataCacheEntries.
/// Reusable by any component that needs persistent caching with GZip compression and TTL.
/// </summary>
public class DbBackedCacheService(IDbContextFactory<DatabaseContext> dbContextFactory)
{
	/// <summary>
	...
	public async Task<T?> GetOrAddAsync<T>(string key, TimeSpan expiry, Func<Task<T?>> factory)
	{
		...
		// Do not cache null results (e.g. transient failures)
		if (value is null)
		{
			return value;
		}
