...
namespace GhostfolioSidekick.PerformanceCalculations.Calculator
{
	public class HoldingPerformanceCalculator(
		...
		IApplicationSettings applicationSettings) : IHoldingPerformanceCalculator
	{
		public async Task<IEnumerable<HoldingAggregated>> GetCalculatedHoldings()
		{
			...
			foreach (var data in holdingData)
			{
				...
				foreach (var accountId in accountIds)
				{
					...
					var currencies = applicationSettings.ConfigurationInstance.Settings.Currencies?.Select(c => Currency.GetCurrency(c)).Where(c => c != Currency.NONE).Distinct().ToList() ?? [];
					currencies.Add(defaultSymbolProfile.Currency);
		...
		Dictionary<(string Symbol, string DataSource), Dictionary<DateOnly, Money>> preLoadedMarketData)
		{
			...
			for (var date = minDate; date <= maxDate; date = date.AddDays(1))
			{
				...
				snapshot.TotalValue = marketPriceConverted.Times(snapshot.Quantity).Amount;
		...
		ICurrencyExchange currencyExchange)
		{
			...
			foreach (var activity in dayActivities)
			{
				...
				if (sign == 1)
				{
					...
					snapshot.TotalInvested = snapshot.TotalInvested + convertedTotal.Amount;
				...
				else
				{
					...
					snapshot.TotalInvested = snapshot.TotalInvested - costBasisReduction;
					snapshot.Quantity -= activity.AdjustedQuantity;
		...
		private static decimal CalculateAverageCostPrice(CalculatedSnapshot snapshot)
		{
			...
			if (snapshot.Quantity < Constants.Epsilon)
			{
