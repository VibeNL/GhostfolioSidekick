...
namespace GhostfolioSidekick.Model.UnitTests.Activities.Types
{
	public class CashDepositWithdrawalActivityTests
	{
		...
		public CashDepositWithdrawalActivityTests()
		{
			...
			activity = new CashDepositWithdrawalActivity(account, dateTime, amount, transactionId);
		}
		...
		{
			// Arrange
			var otherActivity = new CashDepositWithdrawalActivity(activity.Account, activity.Date, activity.Amount, activity.TransactionId);
		...
		{
			// Arrange
			var otherActivity = new CashDepositWithdrawalActivity(activity.Account, activity.Date, new Money(Currency.USD, 5), activity.TransactionId);
