...
namespace GhostfolioSidekick.Model.UnitTests.Activities.Types
{
	public class InterestActivityTests
	{
		...
		public InterestActivityTests()
		{
			...
			activity = new InterestActivity(account, dateTime, amount, transactionId);
		}
		...
		{
			// Arrange
			var otherActivity = new InterestActivity(activity.Account, activity.Date, activity.Amount, activity.TransactionId);
		...
		{
			// Arrange
			var otherActivity = new InterestActivity(activity.Account, activity.Date, new Money(Currency.USD, 5), activity.TransactionId);
