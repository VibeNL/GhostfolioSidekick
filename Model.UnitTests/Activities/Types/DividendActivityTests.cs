...
namespace GhostfolioSidekick.Model.UnitTests.Activities.Types
{
	public class DividendActivityTests
	{
		...
		{
			// Arrange
			var otherActivity = new DividendActivity(activity.Account, activity.Date, activity.Amount, activity.TransactionId, null, null);
		...
		{
			// Arrange
			var otherActivity = new DividendActivity(activity.Account, activity.Date, new Money(Currency.USD, 5), activity.TransactionId, null, null);
