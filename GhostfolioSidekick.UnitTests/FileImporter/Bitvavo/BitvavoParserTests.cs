...
namespace GhostfolioSidekick.UnitTests.FileImporter.Bitvavo
{
	public class BitvavoParserTests
	{
		...
		public async Task ConvertActivitiesForAccount_SingleBuy_Converted()
		{
			...
			api.Setup(x => x.FindSymbolByIdentifier(new string[] { "Storj", "STORJ" }, It.IsAny<Currency>(), It.IsAny<AssetClass[]>(), It.IsAny<AssetSubClass[]>(), true, false)).ReturnsAsync(asset);
			...
			account.Activities.Should().BeEquivalentTo(new[] { new Activity(
				...
				34.75825253M,
				new Money(DefaultCurrency.EUR, 0.71746M, new DateTime(2023, 12, 13, 14, 39, 02, 473, DateTimeKind.Utc)),
				new[] { new Money(DefaultCurrency.EUR, 0.0623441398262M, new DateTime(2023, 12, 13, 14, 39, 02, 473, DateTimeKind.Utc)) },
				"Transaction Reference: [16eed6ae-65f9-4a9d-8f19-bd66a75fc745] (Details: asset STORJ)",
		...
		public async Task ConvertActivitiesForAccount_SingleReceive_Converted()
		{
			...
			api.Setup(x => x.GetAccountByName(account.Name)).ReturnsAsync(account);
			api.Setup(x => x.FindSymbolByIdentifier(new string[] { "Cosmos", "ATOM" }, It.IsAny<Currency>(), It.IsAny<AssetClass[]>(), It.IsAny<AssetSubClass[]>(), true, false)).ReturnsAsync(asset);
			...
			account.Activities.Should().BeEquivalentTo(new[] { new Activity(
				...
				15.586311M,
				new Money(DefaultCurrency.EUR, 42, new DateTime(2023, 10, 13, 22, 38, 36, DateTimeKind.Utc)),
				Array.Empty<Money>(),
				 "Transaction Reference: [af86c3d8-ff57-4866-b6ce-7a549db31eda] (Details: asset ATOM)",
		...
		public async Task ConvertActivitiesForAccount_SingleSell_Converted()
		{
			...
			api.Setup(x => x.GetAccountByName(account.Name)).ReturnsAsync(account);
			api.Setup(x => x.FindSymbolByIdentifier(new string[] { "Cardano", "ADA" }, It.IsAny<Currency>(), It.IsAny<AssetClass[]>(), It.IsAny<AssetSubClass[]>(), true, false)).ReturnsAsync(asset);
			...
			account.Activities.Should().BeEquivalentTo(new[] { new Activity(
				...
				45.802549M,
				new Money(DefaultCurrency.EUR, 0.56714M, new DateTime(2023, 12, 13, 14, 45, 51, 803, DateTimeKind.Utc)),
				new[] { new Money(DefaultCurrency.EUR, 0.04645763986M, new DateTime(2023, 12, 13, 14, 45, 51, 803, DateTimeKind.Utc)) },
				"Transaction Reference: [14ae873a-4fce-4a12-ba0f-387522c67d46] (Details: asset ADA)",
