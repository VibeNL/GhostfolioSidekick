		...
		protected override IEnumerable<PartialActivity> ParseRow(CoinbaseRecord record, int rowNumber)
		{
			...
			switch (record.Type)
			{
				...
				case "Withdrawal":
					...
					break;
				case "Receive":
					yield return PartialActivity.CreateReceive(date, symbolIds, record.PositiveQuantity, id);
					break;
				case "Send":
					yield return PartialActivity.CreateSend(date, symbolIds, record.PositiveQuantity, id);
					break;
				case "Convert":
