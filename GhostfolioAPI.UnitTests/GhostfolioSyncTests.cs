...
namespace GhostfolioSidekick.GhostfolioAPI.UnitTests
{
	public class GhostfolioSyncTests
	{
		...
		public async Task SyncAllActivities_ShouldConvertSendAndReceiveActivities()
		{
			// Arrange
			var account = new Account { Name = "TestAccount", SyncActivities = true };
            var sendReceiveActivity = new ReceiveActivity(account, null, [PartialSymbolIdentifier.CreateGeneric("TEST", Currency.USD)], DateTime.Now, 10, "tx1", null, null)
			{
		...
		public async Task SyncAllActivities_ShouldConvertGiftAssetToBuy()
		{
			// Arrange
			var account = new Account { Name = "TestAccount", SyncActivities = true };
            var giftAssetActivity = new GiftAssetActivity(account, null, [PartialSymbolIdentifier.CreateGeneric("TEST", Currency.USD)], DateTime.Now, 10, "tx1", null, null)
			{
				UnitPrice = new Money(Currency.USD, 100)
			};
		...
		public async Task SyncAllActivities_ShouldConvertRepayBondActivity()
		{
            ...
            var buyActivity = new BuyActivity(account, holding, [PartialSymbolIdentifier.CreateGeneric("TEST", Currency.USD)], DateTime.Now.AddDays(-1), 100, new Money(Currency.USD, 10), "tx0", null, null);
			holding.Activities = [buyActivity];

            var repayBondActivity = new RepayBondActivity(account, holding, [PartialSymbolIdentifier.CreateGeneric("TEST", Currency.USD)], DateTime.Now, new Money(Currency.USD, 1200), "tx1", null, null);
		...
		public async Task SyncAllActivities_ShouldSkipRepayBondActivityWithoutHolding()
		{
			// Arrange
			var account = new Account { Name = "TestAccount", SyncActivities = true };
            var repayBondActivity = new RepayBondActivity(account, null, [PartialSymbolIdentifier.CreateGeneric("TEST", Currency.USD)], DateTime.Now, new Money(Currency.USD, 1200), "tx1", null, null);
			var activities = new List<Activity> { repayBondActivity };
		...
		public async Task SyncAllActivities_ShouldConvertNegativeDividendToFee()
		{
			// Arrange
			var account = new Account { Name = "TestAccount", SyncActivities = true };
            var dividendActivity = new DividendActivity(account, null, [PartialSymbolIdentifier.CreateGeneric("TEST", Currency.USD)], DateTime.Now, new Money(Currency.USD, -50), "tx1", null, null);
