...
namespace GhostfolioSidekick.GhostfolioAPI.API
{
            ...
            ICurrencyExchange currencyExchange) : IApiWrapper
    {
        public async Task CreateAccount(Model.Accounts.Account account)
        {
            ...
            var r = await grpcCall.DoGrpcPost($"api.v1.account", res);
        ...
        public async Task CreatePlatform(Model.Accounts.Platform platform)
        {
            ...
            var r = await grpcCall.DoGrpcPost($"api.v1.platform", res);
            if (!r.IsSuccessStatusCode)
            {
        ...
        {
            var content = await grpcCall.DoGrpcGet(
        ...
        public async Task<List<Model.Activities.Activity>> GetActivitiesByAccount(Model.Accounts.Account account)
        {
            ...
            var content = await grpcCall.DoGrpcGet($"api.v1.order");
        ...
        public async Task UpdateAccount(Model.Accounts.Account account)
        {
            ...
            var content = await grpcCall.DoGrpcGet($"api.v1.account.{existingAccount.Id}.balances");
            ...
            {
                await grpcCall.DoGrpcDelete($"api.v1.account-balance.{item.Id}");
            }
            ...
            foreach (var newBalance in account.Balance)
            {
                ...
                if (existingBalance != null)
                {
                    ...
                    await grpcCall.DoGrpcDelete($"api.v1.account-balance.{existingBalance.Id}");
                    await grpcCall.DoGrpcPost($"api.v1.account-balance", res);
                }
                else
                {
                    await grpcCall.DoGrpcPost($"api.v1.account-balance", res);
        ...
        public async Task SyncSymbolProfiles(IEnumerable<Model.Symbols.SymbolProfile> manualSymbolProfiles)
        {
            ...
            foreach (var manualSymbolProfile in manualSymbolProfiles)
            {
                ...
                if (existingProfile == null)
                {
                    ...
                    var r = await grpcCall.DoGrpcPost($"api.v1.admin.profile-data.{manualSymbolProfile.DataSource}.{manualSymbolProfile.Symbol}", res1);
                    if (!r.IsSuccessStatusCode)
                    {
                ...
                try
                {
                    var r = await grpcCall.DoGrpcPatch($"api.v1.admin.profile-data.{manualSymbolProfile.DataSource}.{manualSymbolProfile.Symbol}", res);
                    if (!r.IsSuccessStatusCode)
                    {
        ...
        {
            var content = await grpcCall.DoGrpcGet($"api.v1.admin.market-data.{symbolProfile.DataSource}.{symbolProfile.Symbol}");
            ...
            foreach (var marketData in list)
            {
                ...
                var r = await grpcCall.DoGrpcPut($"api.v1.admin.market-data.{symbolProfile.DataSource}.{symbolProfile.Symbol}.{marketData.Date:yyyy-MM-dd}", res);
                if (!r.IsSuccessStatusCode)
                {
        ...
        {
            var content = await grpcCall.DoGrpcGet($"api.v1.account");
        ...
        {
            var content = await grpcCall.DoGrpcGet($"api.v1.platform");
        ...
        {
            var content = await grpcCall.DoGrpcGet($"api.v1.admin.market-data");
            ...
            foreach (var f in market?.MarketData
                ...
                .ToList() ?? [])
            {
                content = await grpcCall.DoGrpcGet($"api.v1.admin.market-data.{f.DataSource}.{f.Symbol}");
        ...
        private async Task WriteOrder(Activity activity)
        {
            ...
            await grpcCall.DoGrpcPost(url, await ConvertToBody(activity));
        ...
        private async Task DeleteOrder(Contract.Activity activity)
        {
            ...
            await grpcCall.DoGrpcDelete($"api.v1.order.{activity.Id}");
